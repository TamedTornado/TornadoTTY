...
final class PaneStripStoreTests: XCTestCase {
    ...
    func test_desktop_notification_event_ignores_codex_auto_approval_review_lifecycle_copy() throws {
        let store = WorklaneStore()
        let paneID = try XCTUnwrap(store.activeWorklane?.paneStripState.focusedPaneID)
        ...
        store.handleTerminalEvent(
            ...
            event: .desktopNotification(
                TerminalDesktopNotification(
                    title: "Codex",
                    body: "Automatic approval review denied the requested action."
                )
            )
        )
    ...
    func test_codex_main_needs_approval_title_does_not_promote_running_session_without_permission_hook() throws {
        let store = WorklaneStore(readyStatusDebounceInterval: 0)
        let paneID = try XCTUnwrap(store.activeWorklane?.paneStripState.focusedPaneID)
        store.knownNonRepositoryPaths.insert("/tmp/project")
        ...
        XCTAssertEqual(store.activeWorklane?.auxiliaryStateByPaneID[paneID]?.agentStatus?.state, .running)
        XCTAssertEqual(store.activeWorklane?.auxiliaryStateByPaneID[paneID]?.agentStatus?.interactionKind, .some(.none))
        XCTAssertEqual(store.activeWorklane?.auxiliaryStateByPaneID[paneID]?.presentation.runtimePhase, .running)
        XCTAssertEqual(store.activeWorklane?.auxiliaryStateByPaneID[paneID]?.presentation.statusText, "Running")
    }
    ...
    func test_codex_running_title_with_task_progress_surfaces_progress_status() throws {
        let store = WorklaneStore(readyStatusDebounceInterval: 0)
        let paneID = try XCTUnwrap(store.activeWorklane?.paneStripState.focusedPaneID)
        store.knownNonRepositoryPaths.insert("/tmp/project")
        ...
        store.updateMetadata(
            paneID: paneID,
            metadata: TerminalMetadata(
                title: "Working ⠋ project | Tasks 2/5",
                currentWorkingDirectory: "/tmp/project",
                processName: "codex",
                gitBranch: "main"
            )
    ...
    func test_codex_ready_title_with_task_progress_surfaces_idle_progress_without_ready() throws {
        let store = WorklaneStore(readyStatusDebounceInterval: 0)
        let paneID = try XCTUnwrap(store.activeWorklane?.paneStripState.focusedPaneID)
        store.knownNonRepositoryPaths.insert("/tmp/project")
        ...
        XCTAssertFalse(
            store.activeWorklane?.auxiliaryStateByPaneID[paneID]?.presentation.isReady == true
        )
    }

    func test_codex_ready_title_uses_recent_stored_notification_to_surface_needs_input() throws {
        let store = WorklaneStore(readyStatusDebounceInterval: 0)
        let paneID = try XCTUnwrap(store.activeWorklane?.paneStripState.focusedPaneID)
        ...
        store.handleTerminalEvent(
            ...
            event: .desktopNotification(
                TerminalDesktopNotification(
                    title: "Attention",
                    body: "Question requested: Choose environment"
                )
            )
        )
        store.applyAgentStatusPayload(
    ...
    func test_opencode_idle_after_prelaunch_keeps_idle_progress_visible() throws {
        let store = WorklaneStore(readyStatusDebounceInterval: 0)
        let paneID = try XCTUnwrap(store.activeWorklane?.paneStripState.focusedPaneID)
        ...
        XCTAssertEqual(presentation.taskProgress, PaneAgentTaskProgress(doneCount: 1, totalCount: 3))
        ...
        XCTAssertEqual(paneRow.taskProgress, PaneAgentTaskProgress(doneCount: 1, totalCount: 3))
        XCTAssertFalse(paneRow.isWorking)
    }

    func test_opencode_command_finished_after_idle_clears_progress_context() throws {
        let store = WorklaneStore(readyStatusDebounceInterval: 0)
        let paneID = try XCTUnwrap(store.activeWorklane?.paneStripState.focusedPaneID)
        let exitedPID = Int32.max
        ...
        XCTAssertNil(auxiliaryState?.presentation.taskProgress)

        let worklane = try XCTUnwrap(store.activeWorklane)
        let summary = WorklaneSidebarSummaryBuilder.summary(for: worklane, isActive: false)
        let paneRow = try XCTUnwrap(summary.paneRows.first)
        XCTAssertNil(paneRow.taskProgress)
        XCTAssertFalse(paneRow.isWorking)
    }
    ...
    func test_progress_report_does_not_clear_explicit_kimi_approval_state() throws {
        let store = WorklaneStore(readyStatusDebounceInterval: 0)
        let paneID = try XCTUnwrap(store.activeWorklane?.paneStripState.focusedPaneID)
        ...
        store.applyAgentStatusPayload(
            AgentStatusPayload(
                ...
                origin: .explicitHook,
                toolName: "Kimi",
                text: "StrReplaceFile is requesting approval to edit file:",
                interactionKind: .approval,
                confidence: .explicit,
                sessionID: "kimi-session",
                artifactKind: nil,
    ...
    func test_shell_command_running_does_not_clear_explicit_kimi_approval_state() throws {
        let store = WorklaneStore(readyStatusDebounceInterval: 0)
        let paneID = try XCTUnwrap(store.activeWorklane?.paneStripState.focusedPaneID)
    ...
    func test_user_submitted_input_resumes_explicit_kimi_approval_state() throws {
        let store = WorklaneStore(readyStatusDebounceInterval: 0)
        let paneID = try XCTUnwrap(store.activeWorklane?.paneStripState.focusedPaneID)
