...
| `linux.codex-alias-durable-intent` | DELIVERED | An exact authenticated `codex resume <name-or-ID>` shell launch becomes provisional durable restore intent before Codex reports its canonical session identity. Zentty retains the original structural argv, prefers a later canonical ID, and otherwise restores with the original target instead of silently opening a shell. | [GH-138](https://github.com/TamedTornado/zentty/issues/138); `WorkspaceState::record_submitted_shell_command`; `PaneRestoreDraft::resume_command`; `linux/tests/rust-session-restore`; and [`zentty-linux-dogfood-2026-08-28-codex-alias-intent.md`](design/zentty-linux-dogfood-2026-08-28-codex-alias-intent.md) | High. Named-session intent and canonical reconciliation are platform-neutral lifecycle semantics. | Not shared. Present the two-level intent model and failure recovery behavior rather than a Rust-specific patch. |
...
| `linux.visible-pane-notifications` | DELIVERED | An opt-out notification setting suppresses banners and sounds when the target pane is already displayed in a mapped, non-minimized Zentty window, while preserving the notification inbox and continuing to notify for hidden worklanes and offscreen panes. | [GH-140](https://github.com/TamedTornado/zentty/issues/140); `ApplicationShell::attention_target_is_visibly_displayed`; `desktop_attention_decision`; `linux/tests/rust-attention-inbox`; `linux/tests/rust-notifications-settings`; and [`zentty-linux-dogfood-2026-08-30-visible-pane-notifications.md`](design/zentty-linux-dogfood-2026-08-30-visible-pane-notifications.md) | High. The preference and delivery policy are platform-neutral; window visibility queries are platform-specific. | Not shared. Present the user-facing policy and evidence rather than GTK-specific implementation. |
