...
Status: **COMPLETE at the recorded source head — the coverage gates pass and
feature implementation may resume.**

This is a scope-completeness claim, not a product-completion claim. Current
classification and implementation totals are generated by
`linux/tests/feature-inventory`; they are intentionally not copied into this
document as independently maintained prose.
...
The audit runner now checks all of the following:

1. every `AppCommandID`, `PaneCommand`, embedded CLI `ParsableCommand`, and
   settings section is owned by a feature entry;
...
8. implementation claims require focused and real-product evidence, and prose
   counts and classifications are generated from the machine-readable
   inventory rather than copied by hand.

The validator also rejects owner reassignment against the reviewed public issue
ledger, missing or prematurely closed owner issues, missing dependency issues, removed
acceptance evidence, false `IMPLEMENTED` claims, and stale command registries.
Its focused mutation tests exercise those failures without invoking aggregate
Linux qualification.

To produce both the concise report and a machine-readable receipt:

```sh
linux/tests/feature-inventory \
  docs/design/zentty-linux-feature-inventory.json \
  docs/design/zentty-feature-evidence-2026-08-04.json \
  /tmp/zentty-feature-summary.json
```

The remaining epistemic limitation is explicit: file, registry, documentation,
release, screenshot, test, and issue closure cannot prove that a reviewer never
misunderstood behavior inside a covered source file. Newly discovered source
behavior must therefore add or revise an inventory entry rather than being
